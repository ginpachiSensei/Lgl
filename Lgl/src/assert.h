...
#define ASSERT(x) \
    if (!(x))     \
        __debugbreak(); // Break the debugger from executing
#else
#define ASSERT(x) \
    if (!(x))     \
    {             \
    } // For other platforms we don't trigger any debug functions we just record the assertion failure and report it
#endif
